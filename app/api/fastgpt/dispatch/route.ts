...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      console.error('Dispatch Center API error:', response.status, response.statusText);
      return NextResponse.json(
        { error: `调度中心API错误: ${response.status} ${response.statusText}` },
        { status: response.status }
      );
    }

    const data = await response.json();
    console.log('Dispatch Center API response received');
    
    return NextResponse.json(data);
  } catch (error) {
    console.error('Dispatch Center proxy error:', error);
    return NextResponse.json(
      { error: '服务器内部错误' },
      { status: 500 }
    );
